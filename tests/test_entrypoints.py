...
def test_test_command_uses_current_user_template(
    ...
    plugin.templates.user_defaults["user-1"] = "novel"
...
def test_test_command_without_text_uses_builtin_content(
    ...
    content, template, user_id, is_gif, layout = (
        plugin.pipeline.render_for_layout.await_args.args
    )
...
def test_switch_and_list_commands_update_user_default(
    ...
    assert plugin.templates.user_defaults == {"user-1": "novel"}
...
def test_preview_command_renders_selected_template_without_changing_default(
    ...
    plugin.templates.user_defaults["user-1"] = "classic"
    rendered_image = object()
    plugin.pipeline.render_for_layout = AsyncMock(return_value=rendered_image)
    ...
    plugin.pipeline.render_for_layout.assert_awaited_once_with(
        "雨落在旧街上。", "novel", "user-1", False, "auto"
    )
    assert plugin.templates.user_defaults == {"user-1": "classic"}
    assert results[0].kind == "chain"
...
def test_agent_tool_rejects_empty_content_without_sending(
    ...
    plugin.pipeline.render_for_layout.assert_not_awaited()
    assert event.sent == []
...
def test_template_query_limits_custom_metadata_without_reading_source(
    ...
    monkeypatch.setattr(plugin.templates, "available", lambda: ["custom"])
    monkeypatch.setattr(plugin.actions, "event_template", lambda event: "custom")
    monkeypatch.setattr(
        plugin.template_mgr,
...
def test_agent_tool_handles_message_send_failure_without_recording_context(
    ...
    plugin.pipeline.render_for_layout = AsyncMock(return_value=object())
    event = fake_event_type()
...
def test_hidden_context_only_records_successful_render(
    ...
    plugin.config["enable_hidden_ctx_buffer"] = True
    event = fake_event_type()
    plugin.pipeline.render_for_layout = AsyncMock(return_value=None)
    ...
    assert [
        item["content"]
        for item in plugin.hidden_context._items[event.unified_msg_origin]
    ] == ["成功内容"]
...
def test_hidden_context_is_injected_per_conversation(plugin, fake_event_type) -> None:
    ...
    plugin.hidden_context.record(first_event, "仅属于第一个会话")
...
def test_template_prompt_injection_uses_dynamic_compact_catalog(
    ...
    monkeypatch.setattr(plugin.templates, "available", lambda: ["dynamic"])
    monkeypatch.setattr(plugin.actions, "event_template", lambda event: "dynamic")
    monkeypatch.setattr(
        plugin.template_mgr,
        "get_template_metadata",
        lambda name: {
